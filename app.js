...
// Simple demo hash (NOT secure; for demo only)
...
function setProducts(items){ localStorage.setItem('products', JSON.stringify(items)); }

function getCart(){ try { return JSON.parse(localStorage.getItem('cart')||'[]'); } catch(e){ return []; } }
function setCart(items){ localStorage.setItem('cart', JSON.stringify(items)); }

function getBrands(){ try { return JSON.parse(localStorage.getItem('brands')||'[]'); } catch(e){ return []; } }
function setBrands(items){ localStorage.setItem('brands', JSON.stringify(items)); }

function getOrders(){ try { return JSON.parse(localStorage.getItem('orders')||'[]'); } catch(e){ return []; } }
function setOrders(items){ localStorage.setItem('orders', JSON.stringify(items)); }

function getAdmin(){ try { return JSON.parse(localStorage.getItem('adminUser')||'null'); } catch(e){ return null; } }
function setAdmin(obj){ localStorage.setItem('adminUser', JSON.stringify(obj)); }
function adminLoggedIn(){ return !!localStorage.getItem('admin_session'); }
function setAdminSession(on){ if(on){ localStorage.setItem('admin_session', '1'); } else { localStorage.removeItem('admin_session'); } }
...
function renderProduct(){
  ...
  $("#add-to-cart").addEventListener("click", ()=>{
    ...
    else { cart.push({ id: uid(), productId: product.id, size, qty }); }
    setCart(cart);
    alert("Added to cart!");
  });
...
function renderCheckout(){
  ...
  $("#place-order").addEventListener("click", ()=>{
    ...
    const phoneOk = phone.replace(/\D/g,'').length >= 9; // basic check
    ...
    setOrders(orders);
    setCart([]);
    alert("✅ Order placed! (Demo) — Admin can view it under Orders.");
    location.href = "index.html";
  });
}

// ===== Admin Page (Products, Brands, Orders) =====
function renderAdmin(){
  ...
  function renderBrands(){
    ...
    list.innerHTML = brands.map(b=>`
      <div class="row" style="justify-content:space-between;border-bottom:1px solid var(--border);padding:8px 0">
        <span>${b}</span>
        <button class="btn danger" data-action="del" data-brand="${b}">Delete</button>
      </div>
  ...
  $("#brands-list").addEventListener("click",(e)=>{
    ...
    const brands = getBrands().filter(x=>x!==b);
    setBrands(brands);
    renderBrands();
    refreshBrandSelect();
  });
      ...
      return `
        <div class="cardish" style="margin-bottom:12px">
          ...
          <div style="margin:6px 0">
            <span class="badge">${fullName || "Unnamed"}</span>
            ${cust.email?`<span class="badge">${cust.email}</span>`:""}
            ${cust.phone?`<span class="badge">${cust.phone}</span>`:""}
          </div>
          ${cust.address?`<div class="muted">Address: ${cust.address}</div>`:""}
